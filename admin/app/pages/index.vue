...
type AdminUser = { id: string; email: string; display_name: string; role: 'admin' | 'member'; is_active: boolean; created_at: string; last_login_at?: string }
...
type View = 'overview' | 'people' | 'users' | 'settings'

const config = useRuntimeConfig()
// The admin client is permanently bound to the same-origin private API.
const apiBase = ref(String(config.public.apiBase || '/api'))
const email = ref('')
const password = ref('')
const token = ref('')
...
const newUser = ref({ email: '', display_name: '', password: '', role: 'member' as 'admin' | 'member' })
...
async function loadAdminData() {
  ...
  try {
    const [me, summary, accountList, personList, settingValues] = await Promise.all([
      $fetch<AdminUser>(`${base}/v1/auth/me`, { headers: authHeaders() }),
    ...
    if (me.role !== 'admin') throw new Error('admin role required')
    currentUser.value = me; overview.value = summary; users.value = accountList; people.value = personList; settings.value = settingValues.values
    notice.value = '管理数据已同步。'
  } catch {
    logout('登录已失效或该账号没有管理员权限。')
  }
}
...
onMounted(async () => {
  ...
  if (token.value) await loadAdminData()
})
</script>
...
<template>
  ...
  <main v-else class="admin-shell">
    <aside>
      ...
      <nav><button v-for="item in [{id:'overview',label:'总览'},{id:'people',label:'人物与画像'},{id:'users',label:'账号'},{id:'settings',label:'设置'}]" :key="item.id" :class="{active:activeView===item.id}" @click="activeView=item.id as View">{{ item.label }}</button></nav>
      <button class="logout" @click="logout()">退出登录</button>
    </aside>
    <section class="content">
      <header><div><p class="eyebrow">PRIVATE OPERATIONS</p><h1>{{ {overview:'运行总览',people:'人物与画像',users:'账号与权限',settings:'系统设置'}[activeView] }}</h1></div><span class="status">● 后端已连接</span></header>
      ...
      <template v-else-if="activeView==='users'">
        <article class="panel form-grid"><h3>创建账号</h3><input v-model="newUser.display_name" placeholder="显示名称"><input v-model="newUser.email" inputmode="email" placeholder="邮箱"><input v-model="newUser.password" type="password" placeholder="默认至少 8 位密码"><select v-model="newUser.role"><option value="member">普通成员</option><option value="admin">管理员</option></select><button :disabled="busy" @click="createUser">创建账号</button></article>
        <article v-for="user in users" :key="user.id" class="panel user"><div><strong>{{ user.display_name }}</strong><p>{{ user.email }} · {{ user.role==='admin'?'管理员':'普通成员' }}</p></div><span :class="user.is_active?'enabled':'disabled'">{{ user.is_active?'已启用':'已停用' }}</span><button class="secondary" @click="toggleUser(user)">{{ user.is_active?'停用':'启用' }}</button></article>
      </template>
